-- Implementation of floating terminal was taken from
-- https://github.com/folke/lazy.nvim/blob/aedcd79811d491b60d0a6577a9c1701063c2a609/lua/lazy/view/float.lua
...
function M:init(opts)
  ...
  self:mount()
  self:on({ "BufHidden", "BufLeave", "BufDelete" }, self.close, { once = true })
  return self
end
...
function M:mount()
  ...
  local function opts()
    ...
    wo(self.win, "conceallevel", 3)
    wo(self.win, "foldenable", false)
    wo(self.win, "spell", false)
    wo(self.win, "wrap", true)
    wo(self.win, "winhighlight", "Normal:LazyNormal")
    wo(self.win, "colorcolumn", "")
  end
  opts()
...
function M:close(_)
  ...
  -- Save current session's stdout to process it later
  if buf and vim.api.nvim_buf_is_valid(buf) then
    self.session = vim.api.nvim_buf_get_lines(buf, 0, -1, false)
  end
